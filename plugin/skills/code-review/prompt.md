Review PR {{PR_URL}} at head commit {{HEAD_SHA}}. Output the review artifact at $TERMINAL_REVIEWS_DIR/{{PR_NUMBER}}/{{SHORT_SHA}}.md. If that variable is unset, resolve the directory with `tm-state-dir reviews`. Never write the artifact inside the repo — it is personal workflow state, and a collaborator pulling the branch has no use for it.
...
Follow the contract at $(tm-agent-spec code-review) for the scoring rubric + artifact format — it is this repo's own if it has one, otherwise the shipped default. Score the six axes (correctness, security, architecture, conformance, quality, dependencies). For Security, run the tm security-scan skill in diff mode first (`/tm:security-scan` in Claude Code, `tm-security-scan` in Codex); take the lower of its recommended score and your manual read.
