...
the contract `tm-agent-spec code-review` resolves to — this repo's own if it has one, otherwise the shipped default.
...
```bash
PACKET=$(${CLAUDE_PLUGIN_ROOT}/bin/code-review-preflight "$PR_URL")
EXIT=$?
```
...
sed "s|{{PR_URL}}|$PR_URL|; s|{{HEAD_SHA}}|$HEAD|; s|{{SHORT_SHA}}|$SHORT|; \
  ...
  ${CLAUDE_PLUGIN_ROOT}/skills/code-review/prompt.md > /tmp/cr-prompt-$$.txt
...
STATS=$(${CLAUDE_PLUGIN_ROOT}/bin/findings-merge "$PR" "$HEAD" /tmp/findings-new-$$.json)
...
VERDICT=$(${CLAUDE_PLUGIN_ROOT}/bin/compute-verdict "$PR" /tmp/scorecard-$$.json --test-status "$TEST_STATUS")
...
```bash
if [ "$VERDICT_VAL" = "approve" ] && [ "$MERGE_READY" = "true" ] && [ -x ${CLAUDE_PLUGIN_ROOT}/bin/chunk-diff ]; then
  git diff "origin/$BASE...$HEAD" > "$REVIEW_ROOT/$PR/$SHORT.diff.patch"
  ${CLAUDE_PLUGIN_ROOT}/bin/chunk-diff --patch "$REVIEW_ROOT/$PR/$SHORT.diff.patch" \
  ...
  sed "s|{{PR}}|$PR|; s|{{SHORT}}|$SHORT|; s|{{BASE}}|$BASE|; s|{{HEAD}}|$HEAD|; \
    ...
    ${CLAUDE_PLUGIN_ROOT}/skills/digest/prompt.md > /tmp/cr-digest-$$.txt
  codex exec -s workspace-write -c model_reasoning_effort="low" -C "$PWD" \
    "$(cat /tmp/cr-digest-$$.txt)" < /dev/null

  [ -f "$REVIEW_ROOT/$PR/$SHORT.digest-patch.json" ] && ${CLAUDE_PLUGIN_ROOT}/bin/merge-digest \
