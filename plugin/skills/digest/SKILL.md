...
the contract `tm-agent-spec digest` resolves to.
...
${CLAUDE_PLUGIN_ROOT}/bin/chunk-diff \
...
sed "s|{{PR}}|$PR|; s|{{SHORT}}|$SHORT|; s|{{BASE}}|$BASE|; s|{{HEAD}}|$HEAD|; \
  ...
  ${CLAUDE_PLUGIN_ROOT}/skills/digest/prompt.md > /tmp/digest-prompt-$$.txt
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/merge-digest \
