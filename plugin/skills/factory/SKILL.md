...
Fallback command helpers: `${CLAUDE_PLUGIN_ROOT}/bin/list-agents` and
`${CLAUDE_PLUGIN_ROOT}/bin/request-agent-artifact`.
...
2. Use `list_agents({repo})` or `${CLAUDE_PLUGIN_ROOT}/bin/list-agents`.
