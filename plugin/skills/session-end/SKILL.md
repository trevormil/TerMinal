...
Before filing, run `${CLAUDE_PLUGIN_ROOT}/bin/list-agents` or MCP `list_agents({repo})` and
...
(`${CLAUDE_PLUGIN_ROOT}/bin/status > .status.md`) so it shows no active session. Commit the
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/activity session-end "Session closed · <slug>" "<one-line summary>"
```
