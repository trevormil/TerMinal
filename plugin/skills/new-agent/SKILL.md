...
1. Read enough local context before writing:
   ...
   - `tm-agent-spec forge` if the agent may open a PR/MR
...
# ${CLAUDE_PLUGIN_ROOT}/bin/list-agents
# ${CLAUDE_PLUGIN_ROOT}/bin/request-agent-artifact --agent knowledge-base --title "Question" -- "Prompt..."
```
...
- Use `terminal-cli mcp list_agents` or `${CLAUDE_PLUGIN_ROOT}/bin/list-agents` before filing
  tickets that need an owner agent.
- Use `terminal-cli mcp request_agent_artifact` or
  `${CLAUDE_PLUGIN_ROOT}/bin/request-agent-artifact` for focused cross-domain knowledge
