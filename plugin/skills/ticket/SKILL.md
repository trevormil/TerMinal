...
`${CLAUDE_PLUGIN_ROOT}/bin/list-agents` as the non-MCP fallback for owner-agent selection.
...
- **Agent** — every ticket is assigned to exactly one agent:
  ...
  or `${CLAUDE_PLUGIN_ROOT}/bin/list-agents` otherwise.
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/activity ticket-filed "Ticket filed · #<id>" "<title>" --ticket <id>
${CLAUDE_PLUGIN_ROOT}/bin/hitl "<title>" "<action needed>"  # only for human-only blockers
${CLAUDE_PLUGIN_ROOT}/bin/activity ticket-closed "Ticket closed · #<id>" "<title>" --ticket <id>
```
