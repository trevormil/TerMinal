...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/merge-sync            # dry-run: print the reconcile plan (read-only)
${CLAUDE_PLUGIN_ROOT}/bin/merge-sync --apply    # execute the closes + scrubs
```

It runs one `gh pr list --state merged` (or `glab mr list`, resolved via
`${CLAUDE_PLUGIN_ROOT}/bin/forge`), matches against every ticket's `prs:`, and: closes a ticket
...
(`${CLAUDE_PLUGIN_ROOT}/bin/status > .status.md`) to reflect the closed tickets. Don't
...
```bash
${CLAUDE_PLUGIN_ROOT}/bin/activity ticket-closed "Ticket closed · #<id>" "PR !<iid> merged" --ticket <id> --pr <iid>
```
