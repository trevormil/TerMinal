...
test('timeline selects recorded turn context and soft-cap controls warn and clear', async ({
  ...
  await ux.page.locator('summary').filter({ hasText: 'Burn rate: off' }).click()
  const lookback = ux.page.getByLabel('Context lookback window')
  await expect(lookback).toHaveValue('0')
  await lookback.selectOption('15')
  await ux.page.reload()
  await ux.page.locator('summary').filter({ hasText: 'Burn rate:' }).click()
  await expect(lookback).toHaveValue('15')
  await lookback.locator('..').getByRole('button', { name: 'Clear', exact: true }).click()
  await expect(lookback).toHaveValue('0')
  expect(await ux.failures.rejections()).toEqual([])
})
