...
SESSIONS_DIR=$([ -d "$ROOT/sessions" ] && [ ! -f "$ROOT/.TerMinal/template.json" ] && echo "$ROOT/sessions" || echo "${TERMINAL_SESSIONS_DIR:-$("$HOME/.config/TerMinal/plugin/bin/tm-state-dir" sessions)}")
