Review PR {{PR_URL}} at head commit {{HEAD_SHA}}. Output the review artifact at $TERMINAL_REVIEWS_DIR/{{PR_NUMBER}}/{{SHORT_SHA}}.md, unless this is a legacy v1 repo that already has .reviews/ and no .TerMinal/template.json marker; in that case use .reviews/{{PR_NUMBER}}/{{SHORT_SHA}}.md.
...
Follow .agents/code-review.md for the scoring rubric + artifact format. Score the six axes (correctness, security, architecture, conformance, quality, dependencies). For Security, run the security-scan skill in diff mode first; take the lower of its recommended score and your manual read.
