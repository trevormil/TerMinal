...
function splitLog(body){const lines=String(body||'').split('\n');let fenced=false,at=-1;for(let i=0;i<lines.length;i++){if(/^\s*\x60\x60\x60/.test(lines[i]))fenced=!fenced;else if(!fenced&&lines[i].trimEnd()==='## Log'){at=i;break}}if(at<0)return{prose:String(body||'').trim(),comments:[]};const comments=[];let cur=null,buf=[];const flush=()=>{if(cur)comments.push(Object.assign({},cur,{body:buf.join('\n').trim()}));buf=[]};for(const line of lines.slice(at+1)){const m=line.match(/^### (\S+) · (.+?)$/);if(m){flush();const who=m[2];if(who.indexOf('agent:')===0){const v=who.slice(6).match(/^(.+?)\s+\((.+)\)$/);cur=v?{at:m[1],author:v[1].trim(),kind:'agent',via:v[2].trim()}:{at:m[1],author:who.slice(6).trim(),kind:'agent'}}else cur={at:m[1],author:who.trim(),kind:'human'}}else if(cur)buf.push(line)}flush();return{prose:lines.slice(0,at).join('\n').trim(),comments}}
function commentHeader(c){return '### '+c.at+' · '+(c.kind==='agent'?'agent:'+c.author+(c.via?' ('+c.via+')':''):c.author)}
function ticket(slug,md){const p=parseFm(md),f=p.fm;const s=splitLog(p.body);const runId=String(f.agent_run_id||'').trim();return {slug,id:Number(f.id)||0,title:f.title||slug,status:f.status||'open',priority:f.priority||'medium',horizon:f.horizon||'now',hitl:f.hitl==='true'||f.hitl===true,type:f.type||'feature',source:f.source||'',created:f.created||'',updated:f.updated||'',prs:arr(f.prs),refs:arr(f.refs),depends_on:deps(f.depends_on),acceptance:arr(f.acceptance),agent:ticketAgentFromMeta(f),run:runId?{id:runId,source:f.agent_run_source||'agent',sessionId:f.agent_session_id||undefined,startedAt:f.agent_run_started_at||undefined,status:f.agent_run_status||undefined}:undefined,body:s.prose,comments:s.comments}}
...
function today(){const d=new Date(),p=n=>String(n).padStart(2,'0');return d.getFullYear()+'-'+p(d.getMonth()+1)+'-'+p(d.getDate())}/* LOCAL calendar day, not UTC: matches src/main/local-day.ts */
...
function splitGlobs(s){return String(s||'').split(',').map(g=>g.trim()).filter(Boolean)}
function search(root,q,opts){opts=opts||{};q=String(q||'').trim();if(q.length<2)return [];const flags=['-n','-I','--no-color','--untracked'];if(!opts.caseSensitive)flags.push('-i');if(opts.wholeWord)flags.push('-w');flags.push(opts.regex?'-E':'-F');const inc=splitGlobs(opts.include).map(g=>':(glob)'+g),exc=splitGlobs(opts.exclude).map(g=>':(exclude,glob)'+g),specs=inc.concat(exc);let r=runObj('git',['-C',root,'grep',...flags,'-m','30','-e',q,...(specs.length?['--',...specs]:[])],{cwd:root,maxBuffer:16*1024*1024});let out=r.stdout;if(!out&&r.error){const gflags=['-rnI'];if(!opts.caseSensitive)gflags.push('-i');if(opts.wholeWord)gflags.push('-w');gflags.push(opts.regex?'-E':'-F','-m','30','--exclude-dir=.git','--exclude-dir=node_modules');for(const g of splitGlobs(opts.include))gflags.push('--include='+g);for(const g of splitGlobs(opts.exclude))gflags.push('--exclude='+g);out=runObj('grep',[...gflags,q,root],{cwd:root,maxBuffer:16*1024*1024}).stdout.replaceAll(root+path.sep,'')}return String(out||'').split('\n').filter(Boolean).slice(0,300).map(l=>{const m=l.match(/^(.*?):(\d+):(.*)$/);return m?{file:m[1],line:Number(m[2]),text:m[3].slice(0,240)}:null}).filter(Boolean)}
...
else if(op==='tickets.comment'){const c=input.comment||{},safe=String(input.slug||'').replace(/[^\w-]/g,''),p=areaPaths(root,'backlog').map(dir=>path.join(dir,safe+'.md')).find(exists);if(!p||!String(c.body||'').trim()||!String(c.author||'').trim())out(false);else{let md=fs.readFileSync(p,'utf8');const m=md.match(/^(---\n[\s\S]*?\n---\n?)([\s\S]*)$/);if(!m)out(false);else{const s=splitLog(m[2]);const all=s.comments.concat([{at:c.at||new Date().toISOString(),author:c.author,kind:c.kind==='agent'?'agent':'human',via:c.via,body:String(c.body).trim()}]);const log=all.map(x=>commentHeader(x)+'\n'+x.body).join('\n\n');let head=m[1].replace(/^(updated:[ \t]*).*$/m,'$1'+today());fs.writeFileSync(p,head+'\n'+(s.prose?s.prose+'\n\n':'')+'## Log\n\n'+log+'\n');out(true)}}}
...
else if(op==='files.list')out(listDir(root,input.rel||''));else if(op==='files.read')out(readFile(root,input.rel||''));else if(op==='files.write')out(writeFile(root,input.rel||'',input.content||''));else if(op==='files.search')out(search(root,input.q||'',input.opts));else if(op==='files.create'){const p=safe(root,input.rel||'');if(!p||exists(p))out(false);else{if(input.dir)fs.mkdirSync(p,{recursive:true});else{fs.mkdirSync(path.dirname(p),{recursive:true});fs.writeFileSync(p,'')}out(true)}}else if(op==='files.rename'){const a=safe(root,input.from||''),b=safe(root,input.to||'');if(!a||!b||!exists(a)||exists(b))out(false);else{fs.mkdirSync(path.dirname(b),{recursive:true});fs.renameSync(a,b);out(true)}}else if(op==='files.delete'){const p=safe(root,input.rel||'');if(!p||p===path.resolve(root)||!exists(p))out(false);else{fs.rmSync(p,{recursive:true,force:true});out(true)}}
