...
describe('classifySshError', () => {
  ...
  test('ConnectTimeout expiry also classifies as timeout', () => {
    expect(classifySshError('ssh: connect to host 100.64.0.1 port 22: Connection timed out').reason).toBe('timeout')
  })
  ...
  test('cannot resolve hostname → dns', () => {
    expect(classifySshError('ssh: Could not resolve hostname tm: Name or service not known').reason).toBe('dns')
  })
  test('connection refused → refused', () => {
    expect(classifySshError('ssh: connect to host tm port 22: Connection refused').reason).toBe('refused')
  })
  ...
  test('every reason carries a non-empty actionable hint', () => {
    for (const s of ['timed out', 'Permission denied', 'could not resolve', 'connection refused', 'xyz']) {
