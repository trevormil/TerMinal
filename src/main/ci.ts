...
async function listGh(repoRoot: string, limit: number): Promise<CiListResult> {
  ...
  const runs = arr.map((x): CiRun => {
    ...
    return {
      ...
      updatedAt: updated,
      durationMs:
        status === 'in_progress' || status === 'queued' ? null : Math.max(0, updated - created),
    }
  })
  return { runs }
}
...
async function jobsGh(repoRoot: string, runId: string): Promise<CiJobsResult> {
  const r = await run('gh', ['run', 'view', runId, '--json', 'jobs'], repoRoot, {
    maxBuffer: 8 * 1024 * 1024,
  })
...
async function logGlab(repoRoot: string, jobId: string): Promise<CiLogResult> {
  ...
  if (!repo) return { log: '', error: 'no repo' }
  const proj = encodeURIComponent(repo.path)
  const r = await run('glab', ['api', `projects/${proj}/jobs/${jobId}/trace`], repoRoot, {
