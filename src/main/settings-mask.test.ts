...
import { MASKED_PATHS, SECRET_MASK, maskSettingsSecrets, stripMaskedSecrets } from './settings-mask'
...
const settings = () => ({
  ...
  alerts: { webhook: { enabled: true, url: 'https://hooks.slack.com/services/REAL' } },
})
...
describe('maskSettingsSecrets', () => {
  test('no real secret value survives into the renderer payload', () => {
    ...
    for (const secret of ['sk-or-v1-REALKEY', '12345:REALTOKEN', 'REAL', '999']) {
  ...
  test('masks every path settings.ts seals on disk', () => {
    ...
    expect(masked.alerts.webhook.url).toBe(SECRET_MASK)
    expect(MASKED_PATHS.length).toBe(5)
  })
  ...
  test('non-secret settings and sibling keys pass through untouched', () => {
    ...
    expect(masked.alerts.webhook.enabled).toBe(true)
  })
