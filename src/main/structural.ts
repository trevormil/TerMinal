...
// the local-changes path (local-diff.ts). Kept as a leaf module — it imports
// only ./forge (electron-free) plus node builtins — so both callers and their
// tests can use it without dragging in the electron/IPC surface.
...
export function looksBinary(content: string | null): boolean {
  ...
  return content !== null && content.includes(String.fromCharCode(0))
}

// Run difft on two in-memory content blobs → structural result. Caching is the
// caller's job — the keys differ (MR by headSha, local by working tree). A null
// side means the file is missing there (added/deleted) → an empty file to difft.
