...
/**
 * Claude Code writes control text into the same user/assistant turns a human
 * uses. Rendering it as chat makes the transcript read as if someone said
 * things they never said.
 *
 * `[Request interrupted…]` is worth keeping as a notice — it explains a gap in
 * the conversation. The rest is bookkeeping and goes.
 */
function claudeControlText(text: string): 'notice' | 'drop' | null {
  const t = text.trim()
  if (t.startsWith('[Request interrupted')) return 'notice'
  if (t === 'No response requested.') return 'drop'
  if (t.startsWith('Caveat: The messages below were generated by the user')) return 'drop'
  // Slash-command echoes and injected reminders are wrapped in pseudo-tags.
  if (/^<(command-name|command-message|command-args|local-command-stdout|system-reminder)>/.test(t))
    return 'drop'
  return null
}
...
export function claudeMessages(raw: string): ChatMessage[] {
  ...
  for (const o of parseLines(raw)) {
    ...
    if (o.type === 'user') {
      ...
      if (!text) continue
      const control = claudeControlText(text)
      if (control === 'drop') continue
      out.push(
        control === 'notice'
          ? { kind: 'notice', at, text: text.replace(/^\[|\]$/g, '') }
          : { kind: 'user', at, text },
      )
      continue
    }
    ...
    if (o.type === 'assistant') {
      const text = claudeText(message.content)
      const control = text ? claudeControlText(text) : null
      if (text && control !== 'drop') {
        out.push(
          control === 'notice'
            ? { kind: 'notice', at, text: text.replace(/^\[|\]$/g, '') }
            : { kind: 'assistant', at, text },
        )
      }
