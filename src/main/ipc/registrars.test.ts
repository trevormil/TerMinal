...
describe('IPC registrars', () => {
  ...
  test('index.ts actually calls every registrar — a module nobody wires is dead', () => {
    // The reachability half. `merge-gate.ts` shipped unregistered once: the
    // renderer's override invoke rejected as unhandled, so the audit record it
    // exists to write was never written and nothing said so.
