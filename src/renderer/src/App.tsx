...
export default function App() {
  ...
  const startPairedLoop = async (cfg: PairedLoopConfig): Promise<{ ok: boolean; error?: string }> => {
    ...
    const workerSeed = `/loop-implementer  (paired WORKER, loop ${rec.id}) — state dir: ${stateDir}. This session started in the main repo; do ALL file/code work in the loop worktree ${rec.worktree} (branch ${rec.branch}) — cd into it first. Contract-first: wait for the driver's "contract agreed" before writing code; mark assertions done (never pass). Delivery is automatic — you don't poll; just end each turn by appending your handoff to events.jsonl and act on the driver's messages when they arrive. Goal: ${cfg.goal}`
    const driverSeed = `/loop-driver  (paired DRIVER, loop ${rec.id}) — state dir: ${stateDir}; the worker works in the worktree ${rec.worktree}. You are the operator: FIRST as planner negotiate contract.md into 10–30 testable assertions, THEN as evaluator grade adversarially. Delivery is automatic (TerMinal injects each side's events.jsonl handoff to the other) — end every turn by appending your handoff to events.jsonl; do not let the worker write code until the contract is agreed. Goal: ${cfg.goal}`
