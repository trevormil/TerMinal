  ...
  const providerLine =
    provider === 'github'
      ...
      : provider === 'linear'
        ...
        : provider === 'obsidian'
          ? 'Ticket provider: Obsidian vault. The ticket lives at $OBSIDIAN_TICKETS_DIR/<slug>.md inside the vault at $OBSIDIAN_VAULT_PATH, as markdown with the same frontmatter shape as a local backlog ticket. Update it with the ticket MCP tools or `terminal-cli ticket`, including status and prs: when a PR is opened. Do not create or edit $TERMINAL_BACKLOG_DIR for this ticket.'
  ...
  const instruction =
    provider === 'github'
      ...
      : provider === 'linear'
        ...
        : provider === 'obsidian'
          ? "File exactly ONE new ticket for the request below in the configured Obsidian vault, using this project's ticket conventions. Allocate the next id and write $OBSIDIAN_TICKETS_DIR/NNNN-slug.md (vault at $OBSIDIAN_VAULT_PATH) with valid YAML frontmatter matching the ticket example, put detail in the body after the closing ---, and use the ticket MCP tools or `terminal-cli ticket` rather than editing $TERMINAL_BACKLOG_DIR. Do NOT implement anything or open a PR."
