...
// Hover fill is `--muted` (the neutral hover surface), NOT `--accent` (teal) —
// teal is a brand-emphasis hue, not a generic hover state (§2.1/§2.2).
...
function DropdownMenuCheckboxItem({
  ...
  return (
    <DropdownMenuPrimitive.CheckboxItem
      ...
      className={cn(
        'relative flex cursor-default select-none items-center gap-2 rounded-sm py-1 pl-7 pr-2 text-[12px] outline-none transition-colors focus:bg-muted focus:text-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-40 [&_svg]:size-3.5 [&_svg]:shrink-0',
        className,
      )}
      checked={checked}
      {...props}
...
function DropdownMenuSubTrigger({
  ...
  return (
    <DropdownMenuPrimitive.SubTrigger
      ...
      data-inset={inset}
      className={cn(
        'flex cursor-default select-none items-center rounded-sm px-2 py-1 text-[12px] outline-none focus:bg-muted data-[state=open]:bg-muted data-[inset]:pl-7',
        className,
      )}
      {...props}
    >
