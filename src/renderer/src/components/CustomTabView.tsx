...
import { LayoutDashboard, BarChart3, Boxes, FileText, Gauge, Globe, Server, Table2, type LucideIcon } from 'lucide-react'
...
const iconFor = (name?: string): LucideIcon => (name && ICONS[name.toLowerCase()]) || LayoutDashboard

const IFRAME = 'h-full w-full border-0 bg-white'

function UrlTab({ url }: { url: string }) {
  return <iframe src={url} title={url} className={IFRAME} sandbox="allow-scripts allow-same-origin allow-forms allow-popups" />
}

function CommandTab({ command, cwd, intervalMs }: { command: string; cwd: string; intervalMs?: number }) {
  ...
  if (error) {
    return (
      <div className="flex h-full items-center justify-center bg-[var(--gt-bg)] p-6">
        <pre className="max-w-full overflow-auto whitespace-pre-wrap text-xs text-[var(--gt-red)]">{error}</pre>
      </div>
    )
  }
  if (html === null) {
    return <div className="flex h-full items-center justify-center bg-[var(--gt-bg)] text-xs text-zinc-500">Loading…</div>
  }
  return <iframe srcDoc={html} title={command} className={IFRAME} sandbox="allow-scripts allow-popups allow-forms" />
}

export function CustomTabView({ tab, ctx }: { tab: CustomTab; ctx: TabContext }) {
  if (tab.url) return <UrlTab url={tab.url} />
  if (tab.command) return <CommandTab command={tab.command} cwd={ctx.cwd} intervalMs={tab.intervalMs} />
  return <div className="flex h-full items-center justify-center text-xs text-zinc-500">Empty custom tab (no url or command)</div>
}
