        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="flex items-center gap-1.5 text-[12px] text-zinc-300">
            <input
              type="checkbox"
              checked={opensPr}
              onChange={(e) => setOpensPr(e.target.checked)}
            />
            opens a PR
          </label>
        </div>
...
function PersistentAgentsPanel({ ctx, headerSlot }: { ctx: TabContext; headerSlot?: ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.gt.settings
      .get()
      .then((s) => setCreateEngine(s.defaultEngine))
      .catch(() => {})
  }, [])
  useEffect(() => {
  ...
  const filtered = agents.filter((a) => {
    ...
    if (!q) return true
    return [a.title, a.id, a.description || '', a.tags.join(' ')].some((v) =>
            ...
            <div className="space-y-3">
              {createMode === 'ai' ? (
                <>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {createMsg && (
                      <span className="text-[11px] text-[var(--gt-green)]">{createMsg}</span>
                    )}
                  </div>
                </>
              ) : (
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  disabled={
                    createMode === 'ai' ? !createRequest.trim() || createBusy : !createTitle.trim()
                  }
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (d.kind === 'persistent')
                            localStorage.setItem('gt.persistentAgents.sel', d.ref.id)
                        }}
        ...
        <section className="flex min-w-0 flex-1 flex-col overflow-hidden">
          {(() => {
                  ...
                  <div className="min-h-0 flex-1 overflow-y-auto">
                    ...
                    <section className="p-4">
                      ...
                      <div className="rounded-lg border border-[var(--gt-border)] bg-black/20 p-3 text-[11.5px] leading-relaxed text-zinc-400">
                        ...
                        <code className="font-mono text-zinc-300">artifacts/&lt;run&gt;/</code> and
                        keep memory files current before ending. The memory workspace view exposes
                        the full file browser, artifact previewer, and run controls for this agent.
                      </div>
                    </section>
                  </div>
                </>
            ...
            const activeDefinition =
              selectedDefinition?.kind === 'classic' &&
              selectedDefinition.ref.id === selectedAgent.id
                ? selectedDefinition
                : (definitions || []).find(
                    (d) => d.kind === 'classic' && d.ref.id === selectedAgent.id,
                  ) || null
            return (
              <>
                ...
                <header className="shrink-0 border-b border-[var(--gt-border)] bg-[var(--gt-panel)]/20 px-5 py-4">
                  <div className="flex min-w-0 flex-wrap items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex min-w-0 flex-wrap items-center gap-2">
                        ...
                        {selectedAgent.source && (
                          <Badge tone={SOURCE[selectedAgent.source].tone}>
                    ...
                    <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
                      <button
                        onClick={() =>
                          setPicking({ id: selectedAgent.id, title: selectedAgent.title })
                        }
                        disabled={busy}
                    ...
                    <>
                      ...
                      <section className="border-b border-[var(--gt-border)]/60 p-4">
                          ...
                          meta={
                            <>
                              ...
                              {state?.exists && (
                                <button
                                  onClick={async () => {
                                    if (!confirm('Reset state? Next run will scan from cold.'))
                                      return
                        ...
                        {!state ? (
                          ...
                          (() => {
                            const s = state.state
                            const sha = typeof s.lastScannedSha === 'string' ? s.lastScannedSha : ''
                            const ref = typeof s.lastScannedRef === 'string' ? s.lastScannedRef : ''
                            const at = typeof s.lastRunAt === 'number' ? s.lastRunAt : 0
                            const runId = typeof s.lastRunId === 'string' ? s.lastRunId : ''
                    ...
                    <>
                      ...
                      <section className="border-b border-[var(--gt-border)]/60 p-4">
                        ...
                        {agentRuns.length === 0 ? (
                          <div className="rounded-lg border border-dashed border-[var(--gt-border)] p-4 text-center text-[11px] text-zinc-600">
                            No runs yet. Click{' '}
                            <span className="mx-0.5 font-semibold text-zinc-400">Run</span> above to
                            kick one off.
                          </div>
                        ) : (
                          ...
                          <SectionKicker
                            ...
                            meta={
                              <div className="flex items-center gap-1.5">
                                ...
                                <button
                                  onClick={() =>
                                    window.gt.openExternal(`file://${selectedRun.worktree}`)
                                  }
