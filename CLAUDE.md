...
| An agent's contract (`.agents/<kind>.md`) | `src/main/agent-contracts.ts` + [ADR-0021](./docs/decisions/0021-agent-contracts-ship-with-the-plugin.md). Defaults ship in `plugin/agents/`; a repo carries one only when it overrides. Resolve with `agentContract()` or `tm-agent-spec <kind>` — never a hardcoded path |
